...
void main()
{
    ...
    // // Pattern 15
    // float barX = step(0.4, mod((vUv.x - 0.02) * 10.0, 1.0));
    // barX *= step(0.8, mod(vUv.y * 10.0, 1.0));

    // float barY = step(0.4, mod((vUv.y - 0.02) * 10.0, 1.0));
    // barY *= step(0.8, mod(vUv.x * 10.0, 1.0));
    ...
    // // Pattern 30
    // vec2 lightUv = vec2(vUv.x / 10.0, vUv.y / 2.0);
    // float strength = 0.015 / distance(lightUv, vec2(0.5 / 10.0, 0.5 / 2.0));

    // // Pattern 31
    // vec2 lightUvX = vec2(vUv.x / 10.0, vUv.y / 2.0);
    // float strengthX = 0.015 / distance(lightUvX, vec2(0.5 / 10.0, 0.5 / 2.0));
    // vec2 lightUvY = vec2(vUv.x / 2.0, vUv.y / 10.0);
    // float strengthY = 0.015 / distance(lightUvY, vec2(0.5 / 2.0, 0.5 / 10.0));
    ...
    // vec2 lightUvX = vec2(rotatedUv.x / 10.0, rotatedUv.y / 2.0);
    // float strengthX = 0.015 / distance(lightUvX, vec2(0.5 / 10.0, 0.5 / 2.0));
    // vec2 lightUvY = vec2(rotatedUv.x / 2.0, rotatedUv.y / 10.0);
    // float strengthY = 0.015 / distance(lightUvY, vec2(0.5 / 2.0, 0.5 / 10.0));
    ...
    // );
    // float strength =  1.0 - step(0.01, abs(0.25 - distance(wavedUv, vec2(0.5))));
    ...
    // float angle = atan(vUv.x - 0.5, vUv.y - 0.5) / ( PI * 2.0) + 0.5;
    // vec2 wavedUv = vec2(
    //     vUv.x + cos(angle * 100.0) * 0.02,
    //     vUv.y + sin(angle * 100.0) * 0.02
    // );
    // float strength = 1.0 - step(0.01, abs(0.25 - distance(wavedUv, vec2(0.5))));
    ...
    // float strength = 1.0 - step(0.01, abs(radius - distance(vUv, vec2(0.5))));
